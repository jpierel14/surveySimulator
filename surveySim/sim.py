...
class survey(dict):
    ...
    def _normalize(self,unTargeted=True):
        if unTargeted:
            ...
            # input table must have for each galaxy either
            #   Ncc and Nia:  expected # of SN per obs-frame-year ,  or
            #   SFR: the star formation rate per galaxy
            if ('ncc' in self.galaxies.colnames and
                'nia' in self.galaxies.colnames):
                self.ncc = np.array(self.galaxies['ncc'])
                self.nia = np.array(self.galaxies['nia'])
            elif 'sfr' not in self.galaxies.colnames:
                print('Either NCC + NIa or SFR not found, ' 
                      'provide columns named "NCC" and "NIa" or "SFR"')
                sys.exit(1)
            if not self.mu:
    ...
    def __str__(self):
        ...
        if self.yields:
            ...
            for band in self.yields.keys():
                ...
                print('-------------------')
                print('# band   mlim  NiaLow NiaHi  NCCLow NCChi  NtotLow  NtotHi')
    ...
    def targetedSurvey(self, absolutes={
        'Ia':-19.25, 'Ib':-17.45, 'Ic':-17.66, 'IIP':-16.75},
                       Ia_av=.3, CC_av=.9, zpsys='ab'):
        """
        ...
        if 'ncc' in self.galaxies.colnames:
            ...
            self.galaxies['SNR_CC_upper'] = kcc_upper * \
                                            1E-3 * self.galaxies['sfr']
            self.galaxies['SNR_CC_lower'] = kcc_lower * \
                                            1E-3 * self.galaxies['sfr']
        ...
        if 'nia' in self.galaxies.colnames:
            ...
            self.galaxies['SNR_Ia_lower'] = 1.05E-10 * \
                                            self.galaxies['mass']**.68 + \
                                            kcc_lower * \
                                            self.galaxies['theta'] * \
                                            1E-3 * self.galaxies['sfr']
            self.galaxies['SNR_Ia_upper'] = 1.05E-10 * \
                                            self.galaxies['mass']**.68 + \
                                            kcc_upper * \
                                            self.galaxies['theta'] * \
                                            1E-3 * self.galaxies['sfr']
        else:
            print('Did not find pre-defined SNIa yields or galaxy mass column')
            print('Provide columns "NIa" or "mass."  Instead, using scale factor.')
...
def _SNfraction(classes,band,magLimit,redshifts,cadence,absolutes,mu,Ia_av,CC_av,zpsys):
    """
    (Private)
    Helper function for N_frac.  Defines a dict with keys for each SN class
    requested by the user ('Ia', 'Ib', 'Ic', 'IIP').  The value of each dict
    entry gives the fraction of SN in this class that that would be detected
    by this survey.
    """
