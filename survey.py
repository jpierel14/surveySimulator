...
__all__=['unTargetedSurvey','targetedSurvey']


def _getSFR(z,z0=1.243,A=-.997,B=.241,C=.18):
	return(C/(10**(A*(z-z0))+10**(B*(z-z0))))

def _getSNR(z,kcc_lower=5E-3,kcc_upper=10E-3):
	CSFR=_getSFR(z)
...
def _SNfraction(band,magLimit,redshifts,cadence,absolute_Ia,absolute_CC,mu,Ia_av,CC_av,zpsys):

	classes=['Ia','IIP','Ib','Ic']
	mod,types=np.loadtxt(os.path.join('data','models.ref'),dtype='str',unpack=True)
...
def unTargetedSurvey(area,deltaT,filterList,magLimitList,t_obs=1,galFile=None,dz=.1,zmin=.2,zmax=1.21,absolute_Ia=-19.3,absolute_CC=-17,mu=1,Ia_av=.3,CC_av=.9,zpsys='ab'): #t_obs in years
	try:
		unit=area.unit
	except:
	...
	if not galFile:
		...
		for i in range(len(filterList)):
			_SNfractions=_SNfraction(filterList[i],magLimitList[i],redshifts,deltaT,absolute_Ia,absolute_CC,mu,Ia_av,CC_av,zpsys)
		snYields=dict([])
...
def plotHist(snClass,magLimit,mu,zmin,zmax,dz,bound='lower'):
	fig=plt.figure()
	ax=fig.gca()
	plt.bar(np.arange(zmin,zmax,dz),height=snYield[snClass][bound], width=.1,facecolor='green')
